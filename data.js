...
// ---------- GitHub 同步配置 ----------
const SYNC_CONFIG_KEY = 'industry_tracker_sync_v1';
const SYNC_DEFAULT_REPO_PATH = 'tracker-data.json';

function getSyncConfig() {
  try {
    const raw = localStorage.getItem(SYNC_CONFIG_KEY);
    if (!raw) return getDefaultSyncConfig();
    const cfg = JSON.parse(raw);
    return Object.assign(getDefaultSyncConfig(), cfg);
  } catch (e) {
...
function getDefaultSyncConfig() {
  return {
    ...
    token: '',              // GitHub PAT (Fine-grained, Contents: R/W)
    owner: '',              // 仓库 owner (用户名)
    repo: '',               // 仓库名
    branch: 'main',         // 分支
    path: SYNC_DEFAULT_REPO_PATH, // 数据文件路径
    lastSyncAt: null,       // 上次同步时间
    lastSyncSha: null,      // GitHub 文件 SHA (用于更新)
  };
}

function setSyncConfig(cfg) {
  const merged = Object.assign(getSyncConfig(), cfg || {});
...
window.ST = {
  ...
  getSyncConfig, setSyncConfig, isSyncConfigured,
  SYNC_CONFIG_KEY, SYNC_DEFAULT_REPO_PATH,
